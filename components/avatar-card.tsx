...
export function AvatarCard({ avatar, onClick }: AvatarCardProps) {
  const handleDownload = (e: React.MouseEvent) => {
    e.stopPropagation();
    const svgContent = `<svg class="spkr" viewBox="${avatar.viewBox}">\n  ${avatar.path}\n</svg>`;
  ...
  return (
    ...
    >
      <svg 
        className="spkr w-12 h-12 text-zinc-800 dark:text-zinc-200 mb-3 transition-transform group-hover:scale-110"
